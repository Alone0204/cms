layui.use(['table','layer',"form",'jquery','jquery_cookie'],function(){
    ...
    function delUser(datas){
        ...
        layer.confirm("确定删除选中的记录",{
            ...
            if (bug == true){
                layer.msg("胆子不小啊，boss的账户你也敢删除？",{icon: 0});
                return;
            }
            if (bug2 == true){
                layer.confirm("选择的记录中包含您本人的账户，是否继续删除",{
                    btn:['继续','取消']
                },function (index){
                    layer.close(index);
                    $.ajax({
                        ...
                        success:function (data) {
                            if(data.code==200){
                                ...
                                layer.msg("后会有期，祝您前程似锦！",function () {
                                    // 删除 cookie
                                    c.removeCookie("userIdStr", {domain:"localhost",path:"/cms"});
                                    c.removeCookie("userName", {domain:"localhost",path:"/cms"});
                                    c.removeCookie("trueName", {domain:"localhost",path:"/cms"});
                                    setTimeout(function () {
    ...
    table.on('tool(users)',function (obj) {
        ...
        if(layEvent === "edit"){
            ...
            if (obj.data.userId == userId){
                layer.confirm("确认删除您本人的用户?",{icon: 3, title: "用户管理"},function (index) {
...
//给input标签绑定change事件，一上传选中的.xls文件就会触发该函数
    ...
    function addUserInDatabase(persons) {
        ...
        var y = 0;
        for (var x=0;x<person1.length;x++){
            $.ajax({
                type: "post",
                traditional: true,
                url: url,
