...
public class BrushController extends BaseController {
    ...
    public Map<String,Object> queryAllBrush(HttpServletRequest request){
        return brushService.queryAllBrush(request);
    }
    ...
    public ResultInfo addBrush(Brush brush){
        //参数校验
        AssertUtil.isTrue(brush.getParentId() == null,"请选择或新建大纲！");
        AssertUtil.isTrue(StringUtils.isBlank(DelHtmlUtil.delHTMLTag(brush.getQuestion()))||DelHtmlUtil.delHTMLTag(brush.getQuestion()).trim().equals(""),"请输入你的问题!");
        //设置时间
    ...
    public ResultInfo addAnswer(Answer answer,HttpServletRequest request){
        //参数校验
        AssertUtil.isTrue(StringUtils.isBlank(DelHtmlUtil.delHTMLTag(answer.getAnswer()))||DelHtmlUtil.delHTMLTag(answer.getAnswer()).trim().equals(""),"请输入你的答案!");
