...
public class AnswerService extends BaseService<Answer,Integer> {
    ...
    public void addAnswer(Integer bugId, String answer, HttpServletRequest request) {
        ...
        AssertUtil.isTrue(StringUtils.isBlank(DelHtmlUtil.delHTMLTag(answer))||DelHtmlUtil.delHTMLTag(answer).trim().equals(""),"请输入你的答案!");
        //创建Answer对象
    ...
    public void updateAnswerByBrushIdAndUserId(Answer answer) {
        ...
        AssertUtil.isTrue(StringUtils.isBlank(DelHtmlUtil.delHTMLTag(answer.getAnswer()))||DelHtmlUtil.delHTMLTag(answer.getAnswer()).trim().equals(""),"请输入修改的答案!");
        //调用dao层的修改方法
